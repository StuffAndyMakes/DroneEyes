...
int main(void) {
    ...
    // get stored state from EEPROM
    state = eeprom_read_byte((uint8_t *)0);
    if (state > STATE_MAX) {
    ...
    while(1) {
        ...
        if ((PINB & (1 << PB4)) == 0) {
            ...
            if ((PINB & (1 << PB4)) == 0) {
                ...
                // store it in EEPROM so board remembers user's last setting
                eeprom_write_byte((uint8_t *)0, state);
                // wait for release
                while ((PINB & (1 << PB4)) == 0);
            }
        ...
        // if currently off, don't waste cycles and power below
        if (state == STATE_OFF) continue;
